...
#PER POP
df_pop = 1000*df_sep.div(df.iloc[:,1],axis = 0)
...
def top_ten(df_test,date1=df.columns[-2],num=10,title1=''):
	...
	plt.title(str(title1)+' '+str(date1),size=15)

	for i, v in enumerate(df10[date1]):
		ax.text(v + 0.07, i + 0, str(str(v)[:-2]), color='black',size=15)
...
#MM/DD/YY if DD < 10
top_ten(diff,title1='Daily Cases')
